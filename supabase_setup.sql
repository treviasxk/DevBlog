...
RETURNS TABLE (id int8, pin bool, title text, tags text, date timestamptz, content text) AS $$
BEGIN
    RETURN QUERY EXECUTE format('SELECT blog.id, blog.pin, blog.title, blog.tags, blog.date, SUBSTRING(blog.content, 1, %s) || CASE WHEN LENGTH(blog.content) > %s THEN ''...'' ELSE '''' END AS content FROM blog WHERE blog.title ilike ''%s'' AND blog.tags ilike ''%s'' ORDER BY blog.pin DESC, blog.id DESC LIMIT %s OFFSET %s;', max, max, query, tag, range, next);
END;
$$ LANGUAGE plpgsql;